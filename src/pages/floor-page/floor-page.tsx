...
const FloorPage: React.FC = () => {
 ...
 React.useEffect(() => {
    if (!loading && devices) {
      ...
      let currentMapPointsForce: Point[] = currentMapPointsCases[currentForceLevel-1] || [];

      // Force update the state

      setMapPointsState(
        currentMapPointsForce
      );

     
      
      let colorMapPoints = currentMapPointsCases[currentForceLevel-1];
      ...
      for(let i = 0; i < devices.length; i++) {
        ...
        // look for the point
        const point = colorMapPoints.find(p => p.label === device.device.toString());
        if (point) {
          ...
          console.warn(`Device with id ${device.device} not found in currentMapPoints`);
        }
      }
