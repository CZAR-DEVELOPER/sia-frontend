...
const FloorMapComponent: React.FC<FloorMapComponentProps> = ({ pointsList, imageUrl, currentBuilding = '', currentLevel = ''  }) => {
  
  const handleUrl = (point: Point) => {
    if (point.type === 'uma') {
      return `/uma?building=${currentBuilding}&level=${currentLevel}`;
    } else if (point.type === 'text') {
      return `/vav?building=${currentBuilding}&level=${currentLevel}&section=${point.label}`;
    }
    return '';
  }
...
{pointsList.map((point) =>
        ...
        to={handleUrl(point)}
        key={point.id}
        className={`absolute text-white text-sm md:text-base font-bold cursor-pointer shadow-lg ${point.type === 'uma' ? 'w-15 h-15' : 'w-10 h-10'} rounded-full p-2 backdrop-filter backdrop-blur-sm flex items-center justify-center`}
        style={{
          top: `${point.y}%`,
          left: `${point.x}%`,
